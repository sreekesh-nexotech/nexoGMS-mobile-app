...
bool _isLoadingWeight = true;
  ...
  Future<void> _initializeCache() async {
    await Hive.openBox(_homeCacheBox);
    await Hive.openBox(_weightCacheBox); 
    _loadInitialData();
  }
...
Future<void> _fetchWeightData({bool forceRefresh = false}) async {
    ...
    try {
      final lastSync = Hive.box(_weightCacheBox).get('lastSync');
...
Future<void> _fetchLatestWeight() async {
    final response = await _apiService.authenticatedGet('vital/vitals/current-weight');
...
void _loadCachedWeight() {
    final weight = Hive.box(_weightCacheBox).get('currentWeight', defaultValue: 0.0);
    setState(() => _currentWeight = weight);
  }


 Future<void> _saveWeightData({required double weight, required String lastUpdated}) async {
    await Hive.box(_weightCacheBox).putAll({
...
Future<void> _fetchTargetWeight() async {
  try {
    final response = await _apiService.authenticatedGet('customer/target-weight');
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      setState(() {
  ...
  try {
    final response = await _apiService.authenticatedGet('workout/total-gym-days');
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
...
Future<void> _fetchTodaysWorkout() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() => _todaysWorkout = workouts.isNotEmpty ? workouts[dayIndex] : null);
    }
  } catch (e) {
...
void _navigateToTodaysWorkout() {
  ...
  void _applyCachedData(Map<String, dynamic> cachedData) {
    setState(() {
      ...
      _feeDueDate = cachedData['fee_due_date'] != null 
      ...
      if (cachedData['membership_plan'] != null) {
        ...
        _membershipPlanPeriod = plan['period'] != null ? '${plan['period']} months' : '';
        _membershipPlanAmount = plan['amount'] != null ? '₹${plan['amount']}' : '';
  ...
  try {
    final response = await _apiService.authenticatedGet('customer/check-changes-in-all');
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      ...
      return {
        ...
        if (changedTables.contains('attendance') || 
            changedTables.contains('gym_sessions')) 'gym_data',
        if (changedTables.contains('membership_plan') || 
            changedTables.contains('payments')) 'membership',
      };
    }
    return null;
  } catch (e) {
  ...
  bool _hasRelevantChanges() {
    ...
    final changedTables = _lastKnownChanges!['changed_tables']?.toString() ?? '';
    return changedTables.contains('membership_plan') ||
           changedTables.contains('customer');
  }

  bool _isCacheExpired() {
    final lastSync = Hive.box(_homeCacheBox).get(_lastSyncKey);
  ...
  Future<void> _fetchGymStreak({bool forceRefresh = false}) async {
    if (!forceRefresh) {
      final cachedStreak = Hive.box(_homeCacheBox).get(_gymStreakCacheKey);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await Hive.box(_homeCacheBox).put(_gymStreakCacheKey, _gymStreak);
      } else {
  ...
  Future<void> _fetchFeeDueDate({bool forceRefresh = false}) async {
    if (!forceRefresh) {
      final cachedDate = Hive.box(_homeCacheBox).get(_feeDueDateCacheKey);
      if (cachedDate != null) {
        setState(() {
    ...
    try {
      final response = await _apiService.authenticatedGet('customer/fee-due-date');
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        setState(() {
          _feeDueDate = data['fee_due_date'] != null 
              ? DateTime.parse(data['fee_due_date'])
              : null;
          _isLoadingFee = false;
        });
        await Hive.box(_homeCacheBox).put(
          _feeDueDateCacheKey, 
  ...
  Future<void> _fetchMembershipPlan({bool forceRefresh = false}) async {
    if (!forceRefresh) {
      final cachedPlan = Hive.box(_homeCacheBox).get(_membershipPlanCacheKey);
      if (cachedPlan != null) {
        final data = jsonDecode(cachedPlan);
        setState(() {
          _membershipPlanName = data['plan_name'] ?? 'No plan';
          _membershipPlanPeriod = data['period'] != null ? '${data['period']} months' : '';
          _membershipPlanAmount = data['amount'] != null ? '₹${data['amount']}' : '';
    ...
    try {
      final response = await _apiService.authenticatedGet('customer/customer/flutter/membership-plan');
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        setState(() {
          _membershipPlanName = data['plan_name'] ?? 'No plan';
...
Future<void> _refreshAllData() async {
  try {
    ...
    await Future.wait([
      if (changes?.contains('vitals') ?? true) _fetchWeightData(forceRefresh: true),
  ...
  return Container(
    ...
    child: Column(
      ...
      Row(
        ...
        Padding(
          padding: const EdgeInsets.only(top: 6), // 84 (total Y) - 40 (greeting Y) - 28 (greeting font size ≈ line height)
          child: const Text(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              image: DecorationImage(
                image: AssetImage('assets/images/bird.png'), // Update with your image path
                fit: BoxFit.cover,
              ),
            ),
          ),
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  ...
                  padding: EdgeInsets.zero, // Using exact dimensions instead of padding
  ...
  return Padding(
    ...
    child: GridView.count(
      ...
      children: [
        _buildStatCard(
          title: 'Target Weight',
          value: _isLoadingWeight ? '--' : '${_targetWeight.toStringAsFixed(1)} ',
          secondaryText: 'Kg',
        ...
        _buildStatCard(
          ...
          value: _isLoadingWeight ? '--' : '${_currentWeight.toStringAsFixed(1)} ',
          secondaryText: 'Kg',
          isLoading: _isLoadingWeight,
          valueSize: 24.0,
        ),
      ...
      GridView.count(
        ...
        children: [
          ...
          _buildStatCard(
            ...
            valueColor: _feeDueDate != null ? _getDueDateColor(_feeDueDate!) : Colors.white,
            valueSize: 15.0,
          ),
        ],
      ),
