...
void _validateLoginResponse(Map<String, dynamic> response) {
 ...
 Future<void> _login() async {
  ...
  try {
    developer.log('Attempting login to ${_apiService.baseUrl}/auth/login', 
        name: 'LoginScreen');
    developer.log('Request payload: ${{
    ...
    developer.log('Response status: ${response.statusCode}', name: 'LoginScreen');
    developer.log('Response headers: ${response.headers}', name: 'LoginScreen');
    ...
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(
        builder: (context) => TokenVerificationWrapper(
          baseUrl: _apiService.baseUrl, // Changed from 'baseUrl' to '_apiService.baseUrl'
    ...
    _showError('Network unreachable. Please check:\n'
        '• Server running at ${_apiService.baseUrl}\n'
        '• Your internet connection');
  } on TimeoutException catch (e) {
    developer.log('TimeoutException: ${e.toString()}',
        name: 'LoginScreen', error: e);
    _showError('Connection timeout. Server may be busy');
  } on http.ClientException catch (e) {
    developer.log('ClientException: ${e.toString()}',
        name: 'LoginScreen', error: e);
    _showError('Network error: ${e.message}');
  } on ApiException catch (e) {
    developer.log('ApiException: ${e.statusCode} - ${e.message}',
        name: 'LoginScreen', error: e);
    _showError(e.message);
  } catch (e, stackTrace) {
    developer.log('Unexpected error: ${e.toString()}',
        name: 'LoginScreen', 
        error: e,
  ...
  Future<void> _storeAuthData(String token, String refreshToken, int customerId, int clientId, String customerName) async {
                    ...
                    child: ElevatedButton(
                      onPressed: _isLoading
                          ...
                          : () async {
                              if (!_showOtpField && !_showNewPasswordFields) {
                                ...
                                try {
                                  final response = await http.post(
                                    Uri.parse('${_apiService.baseUrl}/auth/forgot-password'),
                              ...
                              } else if (_showOtpField && !_showNewPasswordFields) {
                                ...
                                try {
                                  final response = await http.post(
                                    Uri.parse('${_apiService.baseUrl}/auth/verify-otp'),
                                    headers: {'Content-Type': 'application/json'},
                                    body: jsonEncode({
...
class _TokenVerificationWrapperState extends State<TokenVerificationWrapper> {
  ...
  final authBox = Hive.box('auth');
  await authBox.clear(); // Clear all auth data, not just access_token
  _redirectToLogin();
}
